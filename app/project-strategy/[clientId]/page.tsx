...
async function getBriefData(clientId: string) {
  ...
  const { data: stageData } = await supabase
    ...
    .in('stage_key', ['project-strategy', 'brand-bible'])
  ...
  return {
    client,
    briefText: fields.get('project-strategy:brief_text') || '',
    approved: fields.get('project-strategy:approved') === 'true',
    // Default to the violet stage accent if the brand bible hasn't been set yet.
    primaryColor: fields.get('brand-bible:primary_color') || '#7C3AED',
    ...
    >
      ...
      <div className="relative h-full flex flex-col justify-between p-12 md:p-16 min-h-[600px]">
        <div>
          ...
          <p className="text-xs uppercase tracking-[0.3em] opacity-60 mt-12 mb-4">The Project Strategy</p>
        </div>
        <div>
          ...
          <p className="text-xl md:text-2xl opacity-80 font-light max-w-2xl">
            Your full project strategy in one document — who we serve, what we discovered, how we'll speak, and what we're building together.
          </p>
        </div>
...
function MethodologyCard({ accent }: { accent: string }) {
  return (
    <div className="bg-stone-900 text-white rounded-3xl p-10 md:p-12 mb-8 print:rounded-none print:break-after-page">
      ...
      <h2 className="text-3xl md:text-4xl font-bold mb-4 leading-tight">How we built this strategy.</h2>
      <p className="text-lg opacity-80 leading-relaxed max-w-2xl mb-8">
        Eight chapters consolidating every piece of research, voice work, and funnel planning into one client-ready kickoff. Every recommendation traces back to evidence — not vibes, not guesses.
      </p>
...
export default async function ProjectStrategyPage({ params }: { params: Promise<{ clientId: string }> }) {
